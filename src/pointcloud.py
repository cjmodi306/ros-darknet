...
class StereoCamera:
	...
	def yolo_callback(self, data):
		...
		#if self.multiple_stereo_data.object_distance[0].object=='person':
			#rospy.loginfo(self.multiple_stereo_data)
		self.pub.publish(self.multiple_stereo_data)
		
		for i in range(len(data.objects)):
			x = data.objects[i].x +400
			y = data.objects[i].y
			w = data.objects[i].w
			h = data.objects[i].h
	...
	def disparity_callback(self,data):
		...
		for cloud in pc2.read_points(data, skip_nans=True):
			points_list.append([cloud[0], cloud[1], cloud[2], cloud[3]])
	...
	def maskImage(self,x,y,x_w,y_h):
		...
		elements = []
		for el in masked_img.compressed():
			if el != 255.0:
				elements.append(el)
			#print(el)
		disparity = np.mean(elements)
		return disparity
	...
	def measureDistance(self,object_type, x,y,w,h):
		...
		'''
		try:
			#return int(self.maskImage(x,y,x+w,y+h))
			return (int((self.focus*self.baseline)*100/self.maskImage(x,y,x+w,y+h)))
		except:
			return -1
	...
	def showImage(self,x,y,x_w,y_h):
		...
		#cv2.putText(self.img, text, (self.x-10, self.y-10), cv2.FONT_HERSHEY_SIMPLEX, 2,(0,0,255), 3)
		cv2.imshow("Image with Object Distance", self.dispImage)
		#print(self.region_of_interest.shape)
		cv2.imshow("Region of interest", self.region_of_interest)
		cv2.waitKey(1)			
...
if __name__=='__main__':
	...
	sc = StereoCamera()
	rospy.Subscriber('/camera/points2', PointCloud2, sc.disparity_callback)
